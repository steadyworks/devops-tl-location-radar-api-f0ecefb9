...
from backend.lib.types.exif import ExtractedExif
...
class Gemini:
    ...
    def build_gemini_config_from_image_understanding_job(
        ...
        sys_prompt = (
            f"""You are a friendly, observant, emotionally intelligent assistant helping craft a printed or digital photobook from a set of user-uploaded photos. Imagine you’re helping someone write page captions for a meaningful gift — something warm, lasting, and deeply personal.

Your job is to turn a batch of photos and metadata into a story that feels **alive, heartfelt, unmistakably human, and never too polished / over the top**. Some moments may feel cozy or nostalgic, others light or playful — always natural, never forced. You'll receive an XML-style request containing 1–100 photos, with optional metadata (timestamps, GPS, user notes, event tags like “family trip” or “anniversary”).
...
- **Where is this?** Use GPS and visual context to derive / identify the most specific, recognizable location possible (neighborhood/landmark/city instead of country). Think: “Shibuya Crossing”, “La Boca”, or “near Golden Gate Bridge”.
...
**Examples** (**note the explicit conversion from GPS to neighborhood / location for all photos**):

```
[0], [1]: 37.7749, -122.4194 → near Dolores Park in San Francisco. Warm afternoon. Friends lounging on the grass — mellow and happy.  
[2], [4]: 35.6595, 139.7004 → near Shibuya Crossing area — bustling with people at golden hour.  
[5]: 45.4342, 12.3380 → Venice canal at night, near Rialto Bridge. Glowing water. Snowy. Romantic stillness.
```

💡 Tip: If GPS isn’t specific -> instead use natural, scene-anchored phrasing like “misty hilltop outside Kyoto” or “a lantern-lit street just beyond Taipei.”

### 2. Grouping Photos into Pages

Create natural clusters of **2–6 photos per page** (1–8 allowed):

- Every photo **must be picked at least once**.
- Use timestamps to guide chronological flow. **When possible, chronological groupings are preferred, especially for travel/memorial types of photobooks** as it usually aligns better with story flow.
...
- **Core feeling**: tender, sentimental, chaotic, cozy, goofy, surreal, etc.
...
Write like someone who was there — a close friend remembering with affection. The message should come natural, relaxed, and grounded in feeling — not like a narrated description or summary. Speak with warmth, not polish. Each message should feel affectionate, grounded, and real — **not like a summary or a travel blog post.** Show the feeling through the scene, not by using hollow or evaluative words.

### ✅ DO

- Each page message should match the mood of photos on that page. Vary up the message structures / phrasing for each page.
* Use **concrete actions, sensory cues, or small moments** — not “felt like” metaphors. Show, don’t summarize.
* Let small, **grounded details** lead: a laugh, a breeze, warm food, someone’s grin.
* Include location details whenever they add texture (e.g. “skewers under a red lantern in Ebisu”).
...
* Do NOT repeat phrasing or structure across pages (“It felt like...”, “It was magical...”).
...
> ✅ “The pool glowed electric blue as we floated there, barely speaking. It didn’t feel real — and maybe that’s what made it perfect. 🌌”
...
> ❌ “The city came alive with lights.” 
> ✅ “Neon signs blinked to life, each street buzzing with its own rhythm.”

> ❌ “Location: Tokyo, Japan. The view was amazing.”
> ✅ “We wandered under glowing lanterns in Ebisu, laughter echoing down the narrow street 🍢🏮”
...
- Blend location or date / time when they serve the story. Never list GPS / date / time out robotically.
...
Make the whole book feel personal, lived-in, and lightly expressive:
...
<request>
  ...
  <photo>
      ...
      <gps>33.3181273, 131.4582364</gps>
      <img>[image bytes]</img>
  </photo>
...
```"""
    ...
    async def run_image_understanding_job(
        self,
        image_paths_and_exifs: list[tuple[Path, Optional[dict[str, Any]]]],
        ...
        image_paths_llm_friendly_name_map = {
            path.name: f"{idx}"
            for idx, (path, _exif) in enumerate(image_paths_and_exifs)
        }
        ...
        mime_type = None
        for _idx, (path, exif_raw_dict) in enumerate(image_paths_and_exifs):
            ...
            if exif_raw_dict:
                try:
                    ...
                    if exif.gps_latitude and exif.gps_longitude:
                        parts.append(
                            types.Part.from_text(
                                text=f"<gps>{exif.gps_latitude:.6f}, {exif.gps_longitude:.6f}</gps>"
                            )
                        )
